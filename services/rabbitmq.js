...
const RABBITMQ_URL = process.env.RABBITMQ_URL || 'amqp://admin:admin@localhost:4002';
const MAX_RETRIES = 3;
...
class RabbitMQService {
    ...
    async listenToClients(callback) {
        try {
            await this.channel.consume(this.queues.clients, (msg) => {
                if (msg !== null) {
                    const content = JSON.parse(msg.content.toString());
                    console.log('Message reçu du service Clients:', content);
                    callback(content);
                    this.channel.ack(msg);
                }
            });
            console.log('Écoute du service Clients activée');
        } catch (error) {
            console.error('Erreur lors de l\'écoute du service Clients:', error);
            throw error;
        }
    }
